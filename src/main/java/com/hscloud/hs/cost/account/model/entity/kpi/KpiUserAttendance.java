...
/**
 * 人员考勤(cost_user_attendance)Model
 *
 * @author you
 * @since 2024-09-13
 */
...
@TenantTable
@Schema(description = "人员考勤表(cost_user_attendance)")
@TableName("kpi_user_attendance")
public class KpiUserAttendance extends Model<KpiUserAttendance> {

    @TableId(value = "id", type = IdType.AUTO)
    @IsAutoIncrement
    @Column(comment = "id", type = MySqlTypeConstant.BIGINT, isNull = false)
    private Long id;
    ...
    @Column(comment = "锁定标记 1:锁定0:未锁定", type = MySqlTypeConstant.CHAR, length = 1, defaultValue = "0")
    ...
    @Column(comment = "删除标记，1:已删除,0:正常,2:人为删除", type = MySqlTypeConstant.CHAR, length = 1, isNull = false, defaultValue = "0")
    ...
    @Column(comment = "数据来源 1.导入 2 采集 3 系统逻辑新增", type = MySqlTypeConstant.CHAR, length = 1)
