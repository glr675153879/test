...
@TenantTable
@Schema(description = "核算项备份")
@TableName("kpi_item_copy")
public class KpiItemCopy extends Model<KpiItemCopy>{
    ...
    @Column(comment="启停用标记，0启用，1停用", type = MySqlTypeConstant.CHAR, length = 1  , isNull = false )
    ...
    @TableField(value = "table_ids", updateStrategy = FieldStrategy.ALWAYS)
    @Column(comment = "核算项基础表ids", type = MySqlTypeConstant.VARCHAR)
    private String tableIds;

    @TableField(value = "item_cond", updateStrategy = FieldStrategy.ALWAYS)
    @Column(comment = "核算项查询条件", type = MySqlTypeConstant.TEXT)
    private String itemCond;

    @TableField(value = "where_sql", updateStrategy = FieldStrategy.ALWAYS)
    @Column(comment = "where条件", type = MySqlTypeConstant.TEXT)
    private String whereSql;

    @TableField(value = "change_version")
    @Column(comment = "转科逻辑版本 0-旧版 1-新版", type = MySqlTypeConstant.CHAR, length = 1, defaultValue = "0")
    private String changeVersion;

    @TableField(value = "equivalent_flag")
    @Column(comment = "是否按当量计算 0-否 1-是", type = MySqlTypeConstant.CHAR, length = 1, defaultValue = "0")
    private String equivalentFlag;

    @TableField(value = "assign_flag")
    @Column(comment = "是否当量分配 0-否 1-是", type = MySqlTypeConstant.CHAR, length = 1,  defaultValue = "0")
    private String assignFlag;

    @TableField(value = "pro_category_code")
    @Column(comment = "项目分类", type = MySqlTypeConstant.VARCHAR, length = 50)
    private String proCategoryCode;

    @TableField(value = "service_item_category_code")
    @Column(comment = "项目类型", type = MySqlTypeConstant.VARCHAR, length = 50)
