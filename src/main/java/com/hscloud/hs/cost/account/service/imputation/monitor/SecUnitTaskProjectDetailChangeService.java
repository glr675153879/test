...
@Slf4j
@Component
public class SecUnitTaskProjectDetailChangeService implements DataChangeMonitor {
    ...
    private static final String UPDATE_DETAIL = "\"%s\"" + "的\"%s\"数量由\"%.2f\"改为\"%.2f\"" + ",标准(元)由\"%.2f\"改为\"%.2f\""
            + ",合计(元)由\"%.2f\"改为\"%.2f\"";


    @Override
    public String getTableName() {
        return ImputationTableEnum.SEC_UNIT_TASK_PROJECT_DETAIL.getCode();
    }
    ...
    public Map<String, Object> dealDataChange(String handleType, ChangeData record) {
        ...
        if(Objects.nonNull(before) && Objects.nonNull(after)){
            ...
            imputationChangeLog.setChangeModel(ChangeModelConstant.UNIT_TASK_PROJECT_DETAIL);
            ...
            if (DataOpEnum.UPDATE.getCode().equals(handleType) && ifUpdate(after, before) &&
                ...
                Long imputationId = Long.parseLong(after.get("prog_project_id") + "");
                ...
                BigDecimal beforeQty = new BigDecimal(before.get("qty").toString()).setScale(2, RoundingMode.HALF_UP);
                BigDecimal afterQty = new BigDecimal(after.get("qty").toString()).setScale(2, RoundingMode.HALF_UP);
                ...
                String format = String.format(UPDATE_DETAIL, hashMapUser.get(after.get("emp_code").toString()), after.get("account_item_name"),
                        beforeQty.doubleValue(),
                        afterQty.doubleValue(),
    ...
    public boolean ifUpdate(Map<String, Object> after, Map<String, Object> before) {
        // 检查特定字段是否发生变化
        List<String> fieldsToCheck = Arrays.asList("qty", "price_value", "amt");
